...
where ho_ten like 'T%' or ho_ten like 'H%'or ho_ten like'K%';
-- 3
...
ORDER BY 
    ...
    from dich_vu dv left join loai_dich_vu ldv on ldv.ma_loai_dich_vu= dv.ma_loai_dich_vu left join hop_dong hd
    on hd.ma_dich_vu=dv.ma_dich_vu left join khach_hang kh on kh.ma_khach_hang=hd.ma_khach_hang
    where hd.ngay_lam_hop_dong not  in (
    select 1
    from hop_dong hd 
    where year(hd.ngay_lam_hop_dong)=2021 and quarter(hd.ngay_lam_hop_dong) in (1,2,3)
    )
...
from dich_vu dv left join loai_dich_vu ldv on ldv.ma_loai_dich_vu= dv.ma_loai_dich_vu left join hop_dong hd
    ...
    -- 8 cách 1
    select kh.ho_ten
    from khach_hang kh group by kh.ho_ten;
    -- cách 2
    ...
    select 1
    from khach_hang kh
    where ho_ten=kh.ho_ten and kh.ma_khach_hang!=ma_khach_hang
    );
    -- 9
    ...
    -- 11 sửa lại
    select dvdk.* from dich_vu_di_kem dvdk left join hop_dong_chi_tiet hdct on hdct.ma_dich_vu_di_kem=dvdk.ma_dich_vu_di_kem left join hop_dong hd on hd.ma_hop_dong=hdct.ma_hop_dong left join
    khach_hang kh on kh.ma_khach_hang=hd.ma_khach_hang left join loai_khach lk on lk.ma_loai_khach=kh.ma_loai_khach
    where lk.ten_loai_khach = 'Diamond' and (kh.dia_chi like '%Vinh%'or kh.dia_chi like '%Quảng Ngãi%');
 -- 12
 select hd.ma_hop_dong,nv.ho_ten,kh.ho_ten,kh.so_dien_thoai,dv.ten_dich_vu,ifnull(sum(hdct.so_luong),0) as tien_dat_coc,hd.tien_dat_coc
 from hop_dong hd left join khach_hang kh on kh.ma_khach_hang=hd.ma_khach_hang left join nhan_vien nv on nv.ma_nhan_vien=hd.ma_nhan_vien
 left join dich_vu dv on dv.ma_dich_vu=hd.ma_dich_vu left join hop_dong_chi_tiet hdct on hdct.ma_hop_dong=hd.ma_hop_dong
 where month(hd.ngay_lam_hop_dong)in (10,11,12)and year(hd.ngay_lam_hop_dong)=2020 and hd.ngay_lam_hop_dong not in(
 (select hd.ngay_lam_hop_dong from hop_dong hd where month(hd.ngay_lam_hop_dong)in (1,2,3,4,5,6) and year(hd.ngay_lam_hop_dong)=2021 ))
 group by hd.ma_hop_dong;
 -- cach 2 
...
group by  hd.ma_hop_dong, nv.ho_ten, kh.ho_ten, kh.so_dien_thoai, dv.ma_dich_vu, dv.ten_dich_vu, hd.tien_dat_coc;
 ...
 from dich_vu_di_kem dvdk left join hop_dong_chi_tiet hdct on hdct.ma_dich_vu_di_kem=dvdk.ma_dich_vu_di_kem left join hop_dong hd on
 hd.ma_hop_dong=hdct.ma_hop_dong left join khach_hang kh on kh.ma_khach_hang =hd.ma_khach_hang
...
-- 14
 ...
 from dich_vu_di_kem dvdk left join hop_dong_chi_tiet hdct on hdct.ma_dich_vu_di_kem=dvdk.ma_dich_vu_di_kem left join hop_dong hd on
 hd.ma_hop_dong=hdct.ma_hop_dong left join dich_vu dv on dv.ma_dich_vu=hd.ma_dich_vu left join loai_dich_vu ldv on ldv.ma_loai_dich_vu=dv.ma_loai_dich_vu left join khach_hang kh on kh.ma_khach_hang =hd.ma_khach_hang
 group by dvdk.ma_dich_vu_di_kem,dvdk.ten_dich_vu_di_kem,ldv.ten_loai_dich_vu
having count(*)=1;
-- 15
select nv.ma_nhan_vien,nv.ho_ten,td.ten_trinh_do,bp.ten_bo_phan,nv.so_dien_thoai,nv.dia_chi,count(*) as hop_dong_toi_da from 
nhan_vien nv left join trinh_do td on td.ma_trinh_do=nv.ma_trinh_do left join bo_phan bp on bp.ma_bo_phan=nv.ma_bo_phan left join hop_dong hd on hd.ma_nhan_vien=nv.ma_nhan_vien
...
where year(hd.ngay_lam_hop_dong) not in(select hd.ngay_lam_hop_dong from hop_dong hd where year(hd.ngay_lam_hop_dong) between 2019 and 2021);
...
-- 20 
select kh.ma_khach_hang as id ,kh.ho_ten,
kh.email,
kh.so_dien_thoai,kh.ngay_sinh,kh.dia_chi
from khach_hang kh
union 
