...
from `order`;
select cus.*,pro.*
from customer cus join `order` o on o.cid=cus.cid join orderdetail od on od.oid=o.oid join product pro on pro.pid=od.pid;
...
from `order` o inner join  orderdetail od on od.oid=o.oid join product p on p.pid=od.pid
